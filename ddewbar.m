function status = ddewbar(t,y,flag,varargin)
...
% initialization / end
  ...
  switch(flag)
  case 'init'               % ddeprint(tspan,y0,'init')
	  hwbar = waitbar(0,'DDE integration','Name','DDE integration');
	  tfinal = t(end);
	  tlast = cputime;
    
  case 'done'               % ddeprint([],[],'done')
	close(hwbar)  
